...
import { momentToMonth, monthToMoment, MONTH_FORMAT } from '../utils/form'
...
export default class MonthPicker extends React.Component {
  ...
  getValueFromProps = props => {
    const { value, disabledMonth } = props
    if ('value' in props && (!disabledMonth || !disabledMonth(value))) {
      return value
    }
    return null
  }
  ...
  onCalendarSelect = (mValue, needClosePanel) => {
    const props = this.props
    const value = momentToMonth(mValue)
    if (!('value' in props)) {
      this.setValue(value)
    }
    if (needClosePanel) {
  ...
  onInputChange = (v, callback) => {
    ...
    const m = monthToMoment(v)
    if ((m || v === '') && !disabledMonth(v)) {
  ...
  render() {
    ...
    const calendar = (
      <Calendar
        locale={zhCN}
        style={{ zIndex: 1000 }}
        formatter={MONTH_FORMAT}
        showDateInput={false}
        value={monthToMoment(value)}
        onSelect={v => this.onCalendarSelect(v, true)}
        onChange={this.onCalendarSelect}
        disabledDate={m => props.disabledMonth(momentToMonth(m))}
