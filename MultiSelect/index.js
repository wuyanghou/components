...
export default class MultiSelect extends React.Component {
  ...
  onMenuDeselect = (val, text) => {
    ...
    if (val === SELECT_ALL_KEY) {
      ...
      value = value.filter(v => v.val !== val)
    }
    this.fireChange(value)
  }
...
/*     if (!open) {
  ...
  getVLForOnChange = value => {
    const props = this.props
    const options = this.getDictOptionsFlat(props.dict)
    const sortedValue = [
      ...value.filter(v => !options.some(o => o.val === v.val)),
      ...options.filter(o => value.some(v => v.val === o.val))
    ]
